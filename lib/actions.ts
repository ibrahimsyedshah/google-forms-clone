...
export async function addFormData(prevState: any, formData: FormData) {
    const rawFormData = Object.entries(Object.fromEntries(formData)).filter(([key, value]) => !key.startsWith("$ACTION")).map(([key, value]) => ({marker: key, value}));
    
    const formIdentifier = rawFormData.find((data) => data.marker === 'identifier')?.value.toString();
    const formDataToSubmit = rawFormData.filter((data) => data.marker !== 'identifier');
